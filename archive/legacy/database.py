...
def ensure_logs_sync_columns(conn: sqlite3.Connection):
    ...
    if "sync_state" in columns:
        cur.execute(
            """
            UPDATE logs
            SET sync_status = COALESCE(NULLIF(sync_status, ''), sync_state, 'pending')
            WHERE COALESCE(NULLIF(sync_status, ''), '') = ''
            """
        )
    conn.commit()
...
def create_default_admin(conn):
    ...
    try:
        cur.execute(
            ...
            ("admin", password_hash, utc_now_text(), "admin"),
        )
        conn.commit()
    except Exception as exc:
        print(f"create admin bad: {exc}")
...
def export_to_csv(conn: sqlite3.Connection, csv_path: str):
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            ["id", "timestamp", "frame_no", "x1", "y1", "x2", "y2", "width", "height", "confidence", "class_id", "label", "crop_path", "model", "video_path"]
        )
...
def create_log_with_sync(
    ...
    return create_log(conn, time_text, event, source, clip, sync_status=sync_status)
...
def update_camera_session(conn: sqlite3.Connection, camera_id: int, is_streaming: bool, frame_timestamp: str = None):
    cur = conn.cursor()
    cur.execute(
        """
        UPDATE camera_sessions
        SET is_streaming = ?, last_frame_timestamp = ?, updated_at = ?
        WHERE camera_id = ?
        """,
        (1 if is_streaming else 0, frame_timestamp, utc_now_text(), camera_id),
    )
    conn.commit()
